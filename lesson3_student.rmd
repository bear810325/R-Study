...
```{r Faceting}
qplot(x = dob_day, data = pf) + scale_x_continuous(breaks=1:31) + facet_wrap(~dob_month, 3)
```
...
qplot(x=friend_count, data = na.omit(pf), binwidth=25) + scale_x_continuous(limits = c(0,1000), breaks = seq(0,1000,50)) + facet_wrap(~gender)
...
qplot(x=tenure/365, data = pf, color = I('black'), fill = I('#099DD9'), binwidth = .25, xlab = 'Number of years using Facebook', ylab = 'Number of users in sample') + scale_x_continuous(breaks = seq(1,7,1), limits = c(0,7))
```
...
qplot(x=friend_count, y=..count../sum(..count..),data = na.omit(pf), binwidth=10, xlab = 'Friend Count', ylab='Proportion of Users with that friend count',geom = 'freqpoly', color = gender) + scale_x_continuous(limits = c(0,1000), breaks = seq(0,1000,50)) 
```
...
# 3rd Qu. means 75% of female users have less than 244 friends.
```
